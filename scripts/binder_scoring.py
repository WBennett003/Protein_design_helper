...
def score_interface(pdb_file, binder_chain="B"):
    ...
    'interface_residue_pdb_ids_str' : interface_residues_pdb_ids_str
    }
...
if __name__ == '__main__':
    pr.init(f'-ignore_unrecognized_res -ignore_zero_occupancy -holes:dalphaball "/user/home/vi21227/work/code/Projects/Protein_design_helper/tools/BindCraft/functions/DAlphaBall.gcc" -corrections::beta_nov16 true -relax:default_repeats 1')
    parser = argparse.ArgumentParser()
    parser.add_argument('--pdbdir')
    parser.add_argument('--output')
    args = parser.parse_args()

    path = pathlib.Path(args.pdbdir)
    designs_files = path.rglob("*.pdb")


    
    data = []
    for design in designs_files:
        interface_scores, interface_AA, interface_residues_pdb_ids_str = score_interface(str(design), 'A')
        design_name = str(design).split('/')[-1]
        print(design_name)
        interface_scores['design_name'] = design_name
        data.append(interface_scores)
    
    binder_df = pd.DataFrame(data)
    binder_df.to_csv(args.output) 